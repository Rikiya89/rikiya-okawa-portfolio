...
const detailsMap: Record<string, ProjectDetails> = {
  ...
  "udemy-business": {
    ...
    role: "役割: WordPress開発 — テーマ改修／検索・絞り込み機能実装／Udemy REST API連携／開発環境整備（Nginx）",
    intro:
      "WordPress/ACFを用いてインタビュー記事サイトを開発。カスタム投稿タイプとフィールド設計を行い、4タクソノミーによる複合絞り込みと、本文・ACF柔軟コンテンツを対象としたフリーワード検索を実装。さらに、講座IDをもとにUdemy REST APIからタイトル・画像・URLを取得し、記事内に自動表示する仕組みを構築。あわせてNginxのリバースプロキシ設定とリライトルール整備を行い、開発環境のルーティングを安定化。",
    responsibilitiesHeading: "主な業務",
    responsibilities: [
      "カスタム投稿タイプ／ACFフィールド設計とWordPressテーマ改修",
      "4タクソノミー（年代・性別・職種・カテゴリ）による複合フィルター実装",
      "ACF柔軟コンテンツを含む、本文＋カスタムフィールド横断の検索ロジック実装",
      "Udemy REST API連携による講座情報の自動取得と記事表示",
      "クリーンURL化とWordPressリライトルール整備",
      "Nginxリバースプロキシ設定による開発環境のルーティング調整",
    ],
    outcomes: [
      "講座ID入力だけで講座情報を自動表示できる仕組みを実装し、入力工数を削減",
      "本文とACFを横断したフリーワード検索により、記事の検索性を向上",
      "複数タクソノミーの絞り込み導線を整理し、目的の記事へ到達しやすい構成を実現",
      "検索処理と表示ロジックを整理し、保守性を向上",
    ],
